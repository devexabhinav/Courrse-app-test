...
export default function Chapters({ className }: { className?: string }) {
    ...
    >
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="text-sm text-gray-600 dark:text-gray-400">
          Total: {chapters.filter((chapter)=> chapter.course_id == courseId).length} chapter{chapters.length !== 1 ? 's' : ''}
        </div>
      </div>
      ...
      {chapters.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {chapters.filter((chapter) => chapter.course_id == courseId).map((chapter: Chapter) => (
            <div
  key={chapter.id}
